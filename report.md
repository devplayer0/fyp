...
abstract: |
  ...
  Automated grading of programming assignments is an increasingly popular
  technique used to reduce manual marking, especially in the face of growing
  class sizes. Additionally, students can benefit from the immediate feedback
  generated by autograding systems.
  The majority of existing autograding techniques are "functional" or
  "black box". Here, a test case is passed to a student's program and the output
  is compared to predetermined correct values.

  This project seeks to develop
  automated grading beyond this limited scope, specifically within the context
  of ARM assembly language programming. Since assembly programs are typically
  written with the goal of being more efficient than those written in languages
  such as Java or Python, performance evaluation is an obvious target for
  automation.

  By developing an accurate simulation of a microcontroller, as well as using a
  real development board, performance metrics can be collected. These metrics
  can then be transformed into a numerical grade and other useful results. A
  flexible configuration system should allow many types of assignments to be
  graded, and for results to be shown to instructors as well as students.

  The final system developed proved to be very successful. Following a demo with
  participation from approximately 50 students, with real assignments, a survey
  showed that feedback generated by the automated performance grading was
  valuable both for grading and as a learning tool.

  \newpage

  # Acknowledgements {.unnumbered .unlisted}

  I'd like to first thank my supervisor, Dr. Jonathan Dukes, for his continuous
  support, encouragement and guidance over the year in developing this project,
  as well as for the opportunity to bring the system to life through a
  demonstration to real students.

  Appreciation for my friends, especially Senán, can't be understated,
  considering the lengthy "discussions" on topics such as ARM reference manuals
