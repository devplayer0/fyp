...
Overall, the system designed, implemented and evaluated for this project proved
...
A small number of submissions (2 - 3 maximum per
assignment) failed to assemble or produce valid results, but this was somewhat
expected. It's likely that differences in the startup code used for the existing
system when compared with the unified firmware and new harnesses caused some
issues for certain students' solutions. There is also a possibility that some
submissions might not have been fully functional with the original tests anyway.
...
assignments. Given the limited number of sample solutions available for testing
before the final deployment, it was quite difficult to determine what log-log
values would be produced and how they should be graded. Figure
\ref{fig:results_arraymove_all} shows that only the "Arrays" assignment gave
any grades less than full marks. Some of the values for this assignment
...
performance evaluation. With all respondents agreeing that it helped in understanding
performance characteristics of their submissions, it's clear that the feedback
generated by Perfgrade is valuable to students. As all responses to the question
on making improvements to performance were very similar, the goal of adding
learning value appears to have been met.
The majority of students agreed that the feedback from the system would also
...
of performance concepts would likely be required to remedy this.

Unsurprisingly, fewer students who responded to the survey
would like enhanced automated grading (including performance evaluation) to
be integrated with future assignments. The potential for marks in assignments
to be lost for programs which perform worse is likely a factor for some. The
majority of students still would like the system however, which might show that
the value added by the system is worth the "risk" of a lower grade.
...
a unified firmware / startup code helped significantly with this goal, along
with the use of pyOCD. Unfortunately it was not possible to bring tracing parity
to the hardware evaluation method.

Perfgrade, the pipeline-based grading configuration system, proved to be
invaluable in combining the performance measurement building blocks for use in
a full autograding deployment. The YAML syntax and ability to embed Python
expressions almost anywhere makes the system very flexible, and it should be
possible to integrate it with autograding platforms other than Submitty in the
future.

The metrics collected and results produced by the system were not perfect, but
are certainly a significant step up over functional autograding. The heatmap
in particular presents a very useful insight into a program's performance.

Successfully carrying out a demo of the project with students submitting
actual assignments was a great achievement which really helped to validate the
results observed from internal testing.
...
The biggest challenge in this project was the attempt to implement hardware
tracing. ARM and STMicroelectronics provide all of the tools and
documentation required to understand and obtain tracing data, so it is
absolutely possible. However, a sort of "stack" of reference manuals (hundreds
or sometimes thousands of pages long) must each be consulted to find the pieces
needed to get a feature like hardware tracing to work. In addition, these
manuals collectively document all of the components, but are not
structured in a way that makes understanding how to use them easy.

## Future work

There is significant scope for further development of this project. Improving
simulator accuracy and achieving hardware tracing are two obvious areas for
future study. Both of these may require a significant extra step however, with
access to ARM IP and cycle-accurate-capable hardware required for each
respectively.

Beyond these improvements, the types of metrics collected and results produced
from them could be significantly expanded. Tracing data in particular should
allow for many opportunities, such as counting specific types of instructions
or measuring space complexity.

The Perfgrade platform could also be further developed to support more step
types or platforms. "Hacks" specific to this project could be cleaned up to make
the system more flexible and grade other types of non-assembly language
assignments.

Finally, the potential for a better web-based autograding platform is clear.
While Submitty is quite extensible, its issues (as documented in this report)
are significant. A carefully designed platform built from the ground up with
modern web technologies would likely be a major improvement.
